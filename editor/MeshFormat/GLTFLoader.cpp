...
Slick::Editor::GLTFScene Slick::Editor::load_gltf(const std::string& fname) {
    ...
    auto gltf = json::parse(fs);

    GLTFScene scene{};
    
    auto parse_primitive = [](json& j) -> GLTFPrimitive {
        GLTFPrimitive prim{};
    ...
    auto parse_data_uri = [](json& j) -> std::vector<u8> {
        std::string data = j;
